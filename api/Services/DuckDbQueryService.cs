    ...
    ILogger<DuckDbQueryService> logger)
{
    public async Task<object> QueryAsync(Guid userId, Guid datasetId, QueryRequest request, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Dashboard widgets set BypassAiBudget: their result goes straight to the browser, not
            // an AI reader, so the AI token-reading budget is irrelevant to them. The row cap
            // (maxRows, above) and the command timeout remain the real safety bounds regardless.
