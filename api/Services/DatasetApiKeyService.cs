...
{
    public async Task<ApiResult<object>> CreateAsync(Guid userId, Guid datasetId, string name, CancellationToken ct)
    {
        ...
        await conn.ExecuteAsync("""
            INSERT INTO dataset_api_keys(id, dataset_id, user_id, name, key_hash)
            VALUES(@Id, @DatasetId, @UserId, @Name, @KeyHash)
            """, new { Id = id, DatasetId = datasetId, UserId = userId, Name = trimmedName, KeyHash = hash });
    ...
    public async Task<ApiResult<object>> ListAsync(Guid userId, Guid datasetId, CancellationToken ct)
    {
        ...
        var rows = await conn.QueryAsync("""
            SELECT id AS api_key_id, name, created_at, last_used_at, revoked_at
